  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button className="bg-brand-blue hover:bg-brand-blue-dark" onClick={() => router.push("/app/leads")}>
            Create Quote
          </Button>
        }
      />
      ...
      {rows.length === 0 ? (
        ...
        <DataTable
          ...
          columns={columns}
          onRowClick={(q: Quote) => {
            if (q.lead_id) router.push(`/app/leads/${q.lead_id}/quotes/${q.id}`);
          }}
        />
      )}
    </div>
