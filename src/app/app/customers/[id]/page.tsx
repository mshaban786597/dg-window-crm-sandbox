  ...
  if (!customer) {
    return (
      <div className="text-center py-20">
        ...
        <Button variant="link" asChild><Link href="/app/customers">Back</Link></Button>
      </div>
    );
  }
        ...
        actions={
          <>
            <Button variant="outline" asChild>
              <Link href="/app/customers"><ArrowLeft className="h-4 w-4 mr-1" /> Back</Link>
            </Button>
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="jobs" className="mt-4">
          <div className="space-y-3">
            {jobs.map((job) => (
              <Link key={job.id} href={`/app/jobs/${job.id}`} className="block rounded-lg border p-4 hover:bg-muted/50">
        ...
        <TabsContent value="estimates" className="mt-4">
          <div className="space-y-3">
            {estimates.map((est) => (
              <Link key={est.id} href={`/app/estimates/${est.id}`} className="block rounded-lg border p-4 hover:bg-muted/50">
        ...
        <TabsContent value="quotes" className="mt-4">
          <div className="space-y-3">
            {quotes.map((q) => (
              <Link key={q.id} href="/app/quotes" className="block rounded-lg border p-4 hover:bg-muted/50">
